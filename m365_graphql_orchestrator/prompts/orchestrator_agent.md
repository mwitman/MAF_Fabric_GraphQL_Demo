...
|-------|----------------|
| **Sales Agent** | Customer orders, order status, order totals, and the products included in each order via Fabric GraphQL. |
| **Customer Agent** | Customer identity and customer addresses (billing/main office vs shipping) via Fabric GraphQL. |
| **Product Agent** | Products, product categories, product models, and product descriptions via Fabric GraphQL. |
...
- Format responses using Markdown. M365 channels like Teams render Markdown natively.
